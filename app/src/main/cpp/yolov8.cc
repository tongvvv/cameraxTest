...
int inference_yolov8_model(rknn_app_context_t *app_ctx, image_buffer_t  *dst_img, letterbox_t  letter_box, object_detect_result_list *od_results)
{
    int ret;
    ...
    // Pre Process
    /*
     * 图像预处理这一步我会在JNI层给处理掉, 直接把处理完的dst_img传进来就行了, 不需要原始图像
    ...
    if (ret < 0)
    {
        LOGI("rknn_outputs_get fail! ret=%d\n", ret);
        return ret;
    }
